...
class Concat(nn.Module):
    ...
    def forward(self, x):
        ...
        target_h = min(t.shape[2] for t in x)
        target_w = min(t.shape[3] for t in x)
...
class SEBlock(nn.Module):
    def __init__(self, channels, internal_neurons=None):
        super(SEBlock, self).__init__()
        if internal_neurons is None:
            internal_neurons = channels // 16

        self.fc1 = nn.Linear(channels, internal_neurons)
        self.fc2 = nn.Linear(internal_neurons, channels)
        self.relu = nn.ReLU()
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        b, c, _, _ = x.size()
        y = torch.mean(x, dim=(2, 3))  # global avg pool
...
# -----------------------------------------------------------------------------
# Compatibility class for submitted checkpoints
# -----------------------------------------------------------------------------
# Some trained checkpoints were saved with a SimSPPF layer referenced as
# models.common.SimSPPF. Keeping this class here allows those checkpoints to be
# loaded for testing/detection without retraining or changing the original
# trained weights.
class SimSPPF(nn.Module):
    """Simplified SPPF block with ReLU activation for checkpoint compatibility."""
    def __init__(self, c1, c2=None, k=5):
        ...
        self.cv1 = Conv(c1, c_, 1, 1, act=activation)
        self.cv2 = Conv(c_ * 4, c2, 1, 1, act=activation)
        self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
